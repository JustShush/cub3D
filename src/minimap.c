...
void	put_square(t_general *gen, int y, int x, int color)
{
	...
	while (j < SCALE)
	{
		...
		{
			if (j == 0 || i == 0)
				my_mlx_pixel_put(gen->img, x + i, y + j, 0x000000, gen);
			else
				my_mlx_pixel_put(gen->img, x + i, y + j, color, gen);
			i++;
		}
		j++;
	}
...
void	put_rays(t_general *gen, int angle, int py, int px)
{
	...
	while (i < SCALE * 3)
	{
		...
		if (rx >= 0 && ry >= 0 && rx <= gen->win_x && ry <= gen->win_y)
			my_mlx_pixel_put(gen->img, rx, ry, RED_PIXEL, gen);
		i++;
	}
}
...
void	put_player(t_general *gen, int py, int px, int r)
{
	...
	if (px >= 0 && py >= 0 && px <= gen->win_x && py <= gen->win_y)
		my_mlx_pixel_put(gen->img, px, py, ORANGE_PIXEL, gen);
	while (i <= 360)
	{
		j = r;
		while (j > 0)
		{
			...
			if (x >= 0 && y >= 0 && x <= gen->win_x && py <= gen->win_y)
				my_mlx_pixel_put(gen->img, x, y, ORANGE_PIXEL, gen);
			j--;
		}
		i += 1;
	}
