...
/*   Updated: 2024/02/26 13:40:56 by ddiniz-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_map_closed(char **map)
{
	...
	i = get_start_map(map);
	if (i + 1 < 0 || !map[i + 1] || first_str(map[i + 1], "1"))
		return (1);
	while (map[++i])
	{
		j = -1;
...
{
	get_textures(gen);
	if (!gen->file || check_map_closed(gen->file) == 1)
	{
		printf("Error - Map is not valid (Map is not closed 2)\n");
		exit_free_check(gen);
	}
	if (!gen->file || check_textures(gen) || check_valid_color(gen)
